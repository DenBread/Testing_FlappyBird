...
void Unity::UnityApplication::OnApplicationCommand(struct android_app *app, int32_t cmd)
{
    ...
    switch (cmd) {
        ...
        {
            ARect insets;
            GameActivity_getWindowInsets(app->activity, GameCommonInsetsType::GAMECOMMON_INSETS_TYPE_DISPLAY_CUTOUT, &insets);
            UNITY_LOG_INFO("GameActivity: window insets change, left: %i right: %i, top: %i, bottom: %i", insets.left, insets.right, insets.top, insets.bottom);
            // using GameActivity_getWindowInsets we can only get the screen safe area, to get
            // cutout bounding boxes need to use JNI. So for now skipping handling of this command
            // and relying on existing JNI listeners in ScreenManagerAndroid
            break;
        }
